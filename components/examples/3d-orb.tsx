...
const Orb: React.FC = () => {
  ...
  // Use a ref for volume so we can access it continuously in the render loop without resetting the loop
  ...
  useEffect(() => {
    ...
    const resetBallMorph = (mesh: THREE.Mesh) => {
      const geometry = mesh.geometry as THREE.BufferGeometry;
      const positionAttribute = geometry.getAttribute("position");
    ...
    const render = () => {
      ...
      // Update morph dynamically in the render loop based on ref
      if (isActiveRef.current && volumeRef.current > 0) {
        updateBallMorph(ballRef.current, volumeRef.current);
      } else if (!isActiveRef.current) {
        // Reset smoothly if not active
        resetBallMorph(ballRef.current);
      }
